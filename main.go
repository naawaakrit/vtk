...
func main() {
	...
	toolbar := container.NewHBox(scanBtn, refreshAllBtn, organizeBtn, playSeriesBtn, renameSeriesBtn, deleteMarkedEpisodesBtn, statsBtn, continueBtn)
...
func (s *appState) showSeriesContextMenu(idx int, ev *fyne.PointEvent) {
	...
	menu := fyne.NewMenu("", playItem, renameItem, deleteItem, deleteMarkedItem)
...
func (s *appState) refreshEpisodeRows() {
	...
	for rootPath, t := range s.tabs {
		...
		for i, ep := range series.Episodes {
			...
			row := container.NewHBox(watchedCheck, label, status, playBtn, renameBtn, delBtn)
